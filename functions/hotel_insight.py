...
def visualize_hotel_features(
    df,
    Hotel_Name,
    top_n=10
):
    """
    Phân tích các từ/cụm từ nổi bật trong review
    tiêu cực và tích cực của một khách sạn.
    ...
    ----------
    df : pandas.DataFrame
        Dataset review khách sạn.

    Hotel_Name : str
        Tên khách sạn cần phân tích.

    top_n : int, default=10
        Số lượng feature nổi bật hiển thị.

    Returns
    -------
    tuple
        (fig_negative, fig_positive)
    ...
    data = df[
        ...
        (df["Score"].notna()) &
        (df["Body_insight"].notna())
    ].copy()


    # ==================================================
    # 2. Separate Negative / Positive
    # ==================================================
    ...
    if n_negative < 10 or n_positive < 10:
        return None, None
    ...
    X_negative = X[:n_negative]

    X_positive = X[n_negative:]
    ...
    negative_tfidf = np.asarray(
        X_negative.mean(axis=0)
    ).ravel()


    positive_tfidf = np.asarray(
    ...
    result["difference"] = (
        result["positive_tfidf"]
        - result["negative_tfidf"]
    )


    # ==================================================
    # 8. Top Negative Features
    # ==================================================
    ...
    # ==================================================
    # 10. Sort for horizontal bar chart
    # ==================================================
    ...
    fig_negative.update_traces(
        ...
        hovertemplate=(
            "<b>%{y}</b><br>"
            "Negative TF-IDF: %{customdata[0]:.4f}<br>"
            "Positive TF-IDF: %{customdata[1]:.4f}<br>"
            "Difference: %{x:.4f}"
            "<extra></extra>"
        )
    )
    ...
    return fig_negative, fig_positive

def visualize_hotel_features_percentile(
    df,
    Hotel_Name,
    top_n=10
):
    """
    Phân tích các từ/cụm từ nổi bật trong nhóm
    review tiêu cực và tích cực dựa trên percentile.
    ...
    p15 = data["Score"].quantile(0.15)

    p85 = data["Score"].quantile(0.85)


    # ==================================================
    # 4. Separate Negative / Positive
    # ==================================================
