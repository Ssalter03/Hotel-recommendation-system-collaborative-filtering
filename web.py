                    ...
                    if selected_rows:
                        with st.spinner("Hệ thống đang phân tích sở thích cá nhân từ mô hình có sẵn..."):
                            try:
                                ...
                                # Chuyển đổi các ID khách sạn được chọn sang Index tương ứng
                                user_pref_raw = spark.createDataFrame(
                                    [(9999, hid, 10.0) for hid in selected_ids],
                                    ["userIndex", "Hotel ID", "Score"]
                                )
                                user_pref = hotel_indexer_model.transform(user_pref_raw).select("hotelIndex").distinct()
                                chosen_indices = [r["hotelIndex"] for r in user_pref.collect()]

                                if not chosen_indices:
                                    st.warning("⚠️ Không thể mapping các ID khách sạn đã chọn với mô hình học máy.")
                                    st.stop()

                                # Lấy thông tin Item Factors từ als_hotel_model
                                item_factors = als_hotel_model.itemFactors
                                
                                # Lọc lấy vector của các khách sạn người dùng đã chọn để tính toán gu khách hàng
                                chosen_factors = item_factors.filter(F.col("id").isin(chosen_indices))
                                
                                # Tính Vector trung bình đại diện cho Sở thích (User Profile Vector) của người dùng hiện tại
                                user_profile = chosen_factors.select(F.array(*[F.avg(F.col("features")[i]) for i in range(als_hotel_model.rank)]).alias("user_features"))
                                user_features_list = user_profile.collect()[0]["user_features"]

                                # Định nghĩa hàm UDF để tính toán khoảng cách Dot Product
                                def dot_product(features):
                                    return float(sum(i*j for i, j in zip(features, user_features_list)))

                                # Dùng chuỗi "float" thay cho F.FloatType()
                                dot_product_udf = F.udf(dot_product, "float")

                                # Tính toán độ tương thích dự đoán nhanh chóng
                                recommendations_df = item_factors.withColumn("predictedScore", dot_product_udf(F.col("features"))) \
                                                                 .select("id", "predictedScore") \
                                                                 .sort(F.col("predictedScore").desc()) \
                                                                 .limit(350)
                                
                                recs_pd = recommendations_df.toPandas()

                                # Bản đồ hóa ID thực tế và Merge thông tin khách sạn
                                ...
                                st.success("Xử lý thành công bằng phương pháp Vector Dot-Product!")
                                st.rerun()

                            except Exception as e:
                                error_details = traceback.format_exc()
                                st.error("❌ Lỗi hệ thống trong quá trình tính toán Vector ALS dựa trên pre-trained model:")
