...
if menu_selection == "📋 Business Problem":
    ...
    col4.metric("RMSE", "0.1897321391159111")

# =========================================================
# MỤC 3: RECOMMENDATION SYSTEM (SO KHỚP PHÂN ĐOẠN ĐỊA CHỈ CHUẨN XÁC)
# =========================================================
    ...
    if hotel_info is not None:
        ...
        city_norm = normalize_text(selected_city) 
        ...
        # NÚT BẤM TÌM KIẾM: Thực hiện xử lý logic NGAY TẠI ĐÂY để kiểm soát Rerun chuẩn xác
        if st.button("🔍 Tìm khách sạn"):
            st.session_state["has_searched"] = True
            
            # 1. Lọc thành phố thông minh (Chỉ quét 2 phân đoạn cuối của địa chỉ)
            def check_city_smart(addr_norm, city_keyword):
                parts = [p.strip() for p in str(addr_norm).split(",")]
                city_area = " ".join(parts[-2:]) if len(parts) >= 2 else " ".join(parts)
                return city_keyword in city_area

            city_condition = hotel_info["Address_norm"].apply(lambda addr: check_city_smart(addr, city_norm))
            ...
            if city_hotels.empty:
                ...
                # 2. Nếu người dùng CÓ NHẬP TÊN ĐƯỜNG
                if street_input.strip():
                    street_norm = normalize_text(street_input).strip() # Giữ nguyên cụm từ khóa liền nhau
                    
                    # HÀM LỌC ĐƯỜNG NGHIÊM NGẶT: Chỉ tìm từ khóa đường ở phần ĐẦU địa chỉ (loại bỏ 3 phân đoạn cuối của Thành phố)
                    def check_street_strict(addr_norm, street_kw):
                        parts = [p.strip() for p in str(addr_norm).split(",")]
                        # An toàn hơn: Loại bỏ phân đoạn cuối (Quốc gia/Thành phố) nếu có để tránh nhiễu chữ "Nha Trang"
                        if len(parts) >= 2:
                        # Loại bỏ tối đa 2 phân đoạn cuối (ví dụ: "Nha Trang", "Việt Nam")
                            street_area = " , ".join(parts[:-2])
                        else:
                            street_area = parts[0]
                            
                        # Tìm kiếm nguyên cụm từ thay vì tìm từng từ đơn lẻ
                        return street_kw in street_area
                    
                    street_condition = city_hotels["Address_norm"].apply(lambda addr: check_street_strict(addr, street_norm))
                    ...
                    # 3. Nếu ĐỂ TRỐNG tên đường -> Lấy toàn bộ thành phố
                    matched = city_hotels
            ...
            st.session_state["show_limit"] = 20 # Reset phân trang mỗi lần bấm tìm kiếm mới
            st.rerun()
        ...
        if st.session_state["has_searched"]:
            ...
            if st.session_state["matched_hotels"] is not None and not st.session_state["matched_hotels"].empty:
                ...
                if st.button("🚀 OK - Nhận gợi ý cá nhân hóa", type="primary"):
                    ...
                    if selected_rows:
                        with st.spinner("AI đang phân tích sở thích cá nhân của bạn..."):
                            ...
                            try:
                                ...
                                if st.session_state.get("is_strict_street"):
                                    saved_keywords = st.session_state.get("search_keywords", [])
                                    if saved_keywords:
                                        # Ở đây cũng áp dụng bộ lọc đường nghiêm ngặt cho phần đầu địa chỉ đề xuất
                                        strict_cond = recs_expanded["Address_norm"].apply(lambda addr: check_street_strict(addr, saved_keywords))
                        ...
                        st.warning("Vui lòng tick chọn ít nhất 1 khách sạn để hệ thống AI nhận diện được gu của bạn.")
